...
def genRandBacon(num, quote, mode):
    ...
    if mode=="W":
        x["operation"]="words"
        x["points"]=450
        x["abMapping"]=random.choice(["ABABABABABABABABABABABABAB","AAAAAAAAAAAAABBBBBBBBBBBBB","BBBBBBBBBBBBBAAAAAAAAAAAAA"])
        x["texta"]="A"
        x["textb"]="B"
        return x
...
def RSA(num, enc):
    ...
    if enc=="D":
        ...
        x["encrypted"]=enc
        x["name2"]="Jason"
        x["question"]="<p>Given (n,c,d)=("+str(n)+","+str(enc)+","+str(d)+"), compute the original message m.</p>"
    return x
...
def genTest(na, preset):
    ...
    if preset=="1":
        ...
        random.shuffle(enc)
        random.shuffle(bac)
        l = ["1 0","1 1","1 2","1 2","1 2","1 2","2 2","2 0","3 D", "4 D", "5 D", "6 D", enc[0], enc[1], "8 1", bac[0], bac[1], "11 D","12 D"]
        n = 19
    elif preset=="4":
        l = ["1 2"]*20
        n = 20
...
@bot.command(name='fetch', help='Fetches a test that has been previously generated. \n`c!fetch Example`')
